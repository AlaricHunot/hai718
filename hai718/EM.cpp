...
int main(int argc, char* argv[]) {
	...
	// Lecture du nombre de gaussiennes à optimiser
	long lNbGaussienne = 0;
	if(argc > 2)
		sscanf(argv[2],"%ld",&lNbGaussienne);
	if(lNbGaussienne <= 0 || lNbGaussienne >= 256)
		lNbGaussienne = 3; // valeur par défaut
	
	// Lecture de la précision à laquelle arrêter l'optimisation
	double dAccuracy = 0;
	if(argc > 3)
		sscanf(argv[3],"%lf",&dAccuracy);
	if(dAccuracy <= 0)
		dAccuracy = 0.0001; // valeur par défaut
